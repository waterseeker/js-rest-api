...
module.exports = (function() {
    ...
    articleRoutes.get('/api/articles', (req, res) => {
        ...
        if (authenticatedUser) {
            const filteredArticles = db.articles.filter(article => article.visibility === 'public' || 
                                                        ...
                                                        (article.visibility === 'private' && article.user === req.session.user.user_id));
    ...
    articleRoutes.post('/api/articles', bodyParser,(req, res) => {
        const headerToken = req.session['authenticationHeader']; 
        const authenticatedUser = db.users.find(u => u.tokens.includes(headerToken));
        ...
        if (authenticatedUser) { 
            ...
            const article = new Article(req.body.content, req.body.visibility, user_id, req.body.title, article_id);
